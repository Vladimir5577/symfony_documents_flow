...
final class DocumentCommentService
{
    ...
    public function create(Document $document, User $user, string $body, array $uploadedFiles): DocumentComment
    {
        ...
        foreach ($uploadedFiles as $uploaded) {
            $this->assertValidUploadedFile($uploaded);
        }
    ...
    private function canManageComment(DocumentComment $comment, User $user): bool
    {
    ...
    private function assertValidUploadedFile(UploadedFile $uploaded): void
    {
        if (!$uploaded->isValid()) {
    ...
    private function sendCommentNotifications(Document $document, User $commentAuthor): void
    {
        ...
        $authorName = $this->presenter->presentUserBrief($commentAuthor)['fullName'] ?? $commentAuthor->getLogin();
        $documentTitle = $document->getName() ?? '';
        $creatorId = $creator?->getId();

        foreach ($recipients as $recipient) {
            $routeName = ($creatorId !== null && $recipient->getId() === $creatorId)
                ? 'app_view_outgoing_document'
                : 'app_view_incoming_document';
            $link = $this->urlGenerator->generate($routeName, ['id' => $document->getId()]) . '#document-comments';
