...
## 4. Скачать PDF заявки

    GET /spa/api/contract-applications/{id}/pdf

Собирает PDF со всеми полями заявки и вложенными файлами (как кнопка
«Скачать PDF» в старом PHP). Отдаёт `application/pdf` с
`Content-Disposition: attachment`. `404`, если заявки нет.

## 5. Скачать / открыть файл
