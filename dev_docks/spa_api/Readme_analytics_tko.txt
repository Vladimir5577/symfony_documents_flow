...
понедельные суммы/счётчики с разбивкой по полигонам (reports).
...
Поле metrics — массив описаний строк таблицы, в фиксированном порядке:
  ...
  key                       label                   unit   type   aggregate*
  ------------------------- ----------------------- ------ ------ -----------
  garbage_trucks_volume     Мусоровозы              м³     num    sum
  garbage_trucks_weight     Вес ТКО мусоровозы      т      num    sum
  containers_volume         Контейнеры              м³     num    sum
  scrap_trucks_volume       Ломовозы                м³     num    sum
  containers_scrap_weight   Вес ТКО конт., ломов    т      num    sum
  vegetation_volume         Растительные            м³     num    sum
  construction_volume       Строительные            м³     num    sum
  terminal_volume           Терминал                руб.   num    sum
  machinery_work            Работа техники          дн.    text   days_count
  smoke                     Задымление              дн.    text   days_count
  fire_without_mchs         Пожар без вызова МЧС    дн.    text   days_count
  fire_with_mchs            Пожар с вызовом МЧС     дн.    text   days_count
  irrigation                Орошение                дн.    text   days_count

  unit      : единица измерения. Отдаётся во всех metrics и в reports-узлах
              /summary.
...
Источник метрик — App\Service\Analytics\TkoMetrics::METRICS (единый список для
SPA и серверной Twig-страницы). Каждый элемент metrics содержит все поля:
key, label, name, unit, type. Значения всегда отдаются строками. Для num пустое
значение = "" (нет данных).
...
Query-параметры:
  ...
  polygon_id  (int, опционально)
                ...
                Список активных полигонов отсортирован по sortOrder ASC, затем
                по name ASC.
...
{
  "polygons":          [{ "id": int, "name": string }, ...],  // все активные, sortOrder→name
...
Query-параметры:
  ...
  limit        (int, по умолчанию 3, макс. 12) — размер среза weeks.
               Значение < 1 → дефолт 3; > 12 → 12.
  offset       (int, по умолчанию 0) — пагинация weeks «от свежих к старым».

Примеры URL:

  /spa/api/analytics/tko/summary
  /spa/api/analytics/tko/summary?limit=3&offset=0
  /spa/api/analytics/tko/summary?limit=3&offset=3
...
Правила weeks:
- Пагинация «от свежих к старым»: limit=3, offset=0 — последние 3 недели из
  availableWeeks; offset сдвигает окно к более старым неделям.
...
================================================================================
  ...
  Контроллер : src/Controller/SpaApi/Analytics/TkoAnalyticsController.php
                 (тонкий — читает query-параметры и зовёт сервис)
                 ::index()   — /spa/api/analytics/tko
                 ::summary() — /spa/api/analytics/tko/summary
  Сервис     : src/Service/Analytics/TkoAnalyticsService.php
                 ::buildWeekGrid() — сборка ответа п.1 (недельная сетка)
                 ::buildSummary()  — сборка ответа п.2 (availableWeeks + weeks)
                 ::buildSeries(), ::paginateWeeks() — внутренняя сборка reports/пагинация
  Метрики    : src/Service/Analytics/TkoMetrics.php
                 ::METRICS, ::DOW — единый источник для SPA и Twig-страницы
  Репозиторий: src/Repository/Analytics/TKO/AnalyticsTKORepository.php
                 ::findByPolygonAndDateRange()   — данные недели по дням (п.1)
                 ::findAvailableWeeks()           — список недель (п.2)
                 ::aggregateWeeklyByPolygon()     — понедельные агрегаты по полигонам (п.2)
  Сущность   : src/Entity/Analytics/TKO/AnalyticsTKO.php

Серверная Twig-страница (заполнение/сохранение): /analytics/tko,
src/Controller/Analytics/TKO/AnalyticsTKOController.php — использует тот же
TkoMetrics::METRICS, поэтому набор метрик SPA и Twig-страницы идентичен.
