================================================================================
SPA API — Clients Department Analytics
================================================================================

Эндпоинт для получения отчётов «Абонентский отдел» в виде дерева метрик
...
отличается только категория метрик (`clients_department`) и состав дерева.
...
--------------------------------------------------------------------------------
  ...
  Method : GET
  URL    : /spa/api/analytics/clients-department/reports
...
Примеры URL:
  ...
  /spa/api/analytics/clients-department/reports?org_id=20

  // следующая страница (предыдущие 12 недель)
  /spa/api/analytics/clients-department/reports?org_id=20&offset=12

  // апрель–май 2026, все недели в диапазоне
  /spa/api/analytics/clients-department/reports?org_id=20&from=2026-04-01&to=2026-05-31&limit=100

  // только одна последняя неделя
  /spa/api/analytics/clients-department/reports?org_id=20&limit=1

curl:

  curl -H "Authorization: Bearer <token>" \
       "https://example.local/spa/api/analytics/clients-department/reports?org_id=20&from=2026-04-01"
...
fetch:
  ...
  const res = await fetch(
    `/spa/api/analytics/clients-department/reports?${params}`,
...
Имя поля `weeks` отражает текущую конфигурацию доски «Абонентский отдел»
...
- Категория метрик жёстко зафиксирована в эндпоинте как `clients_department`.

--------------------------------------------------------------------------------
3. СТРУКТУРА ДЕРЕВА
--------------------------------------------------------------------------------

Состав дерева определяется составом версии доски («Абонентский отдел»),
к которой привязан отчёт (`analytics_metrics.category = 'clients_department'`),
поэтому любые правки в админке (`/analytics/admin/board/...`) сразу отражаются
...
- Эндпоинт возвращает **только** метрики абонентского отдела
  (category = 'clients_department'). Для других категорий (finance, hr, tko, ...)
...
Плоский список подтверждённых отчётов «Абонентский отдел» — для экрана-индекса
на фронте. Дерева метрик в ответе нет; за детализацией клиент обращается
к основному эндпоинту /spa/api/analytics/clients-department/reports выше.

Доска определяется по analytics_boards.category = 'clients_department' (одна
доска на категорию). В выборку попадают только отчёты со status = 'confirmed'.

  Method : GET
  URL    : /spa/api/analytics/clients-department/reports/list
...
Пример URL:
  ...
  /spa/api/analytics/clients-department/reports/list?org_id=20&page=1&per_page=20
  /spa/api/analytics/clients-department/reports/list?from=2026-04-01&to=2026-05-31&per_page=50
...
--------------------------------------------------------------------------------
  ...
  Контроллер : src/Controller/SpaApi/Analytics/ClientsDepartmentAnalyticsController.php
               методы reports() и reportsList()
  Сервис     : src/Service/Analytics/ClientsDepartmentReportTreeService.php
