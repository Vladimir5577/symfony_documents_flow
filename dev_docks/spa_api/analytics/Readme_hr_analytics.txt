...
Плоский список подтверждённых HR-отчётов — для экрана-индекса на фронте.
Дерева метрик в ответе нет; за детализацией клиент обращается к основному
эндпоинту /spa/api/analytics/hr/reports выше.

Доска определяется по analytics_boards.category = 'hr' (одна доска на
категорию). В выборку попадают только отчёты со status = 'confirmed'.

  Method : GET
  URL    : /spa/api/analytics/hr/reports/list
...
Пример URL:
  ...
  /spa/api/analytics/hr/reports/list?org_id=20&page=1&per_page=20
  /spa/api/analytics/hr/reports/list?from=2026-04-01&to=2026-05-31&per_page=50
...
--------------------------------------------------------------------------------
  ...
  Репозитории: src/Repository/Analytics/AnalyticsReportValueRepository.php
               (findReportsWithMetricTree — общий с финансовой аналитикой
                и обращениями граждан, используется для дерева метрик)
