...
================================================================================
  Статус: В ПРОЦЕССЕ РЕАЛИЗАЦИИ (настройка инфраструктуры и пакетов бэкенда)
...
[V] БД канбана спроектирована и миграции готовы.
[V] Бизнес-правила (1 assignee, запрет удаления непустых сущностей) реализованы.
================================================================================
...
--------------------------------------------------------------------------------
[V] 2.1 Аутентификация / права (см. раздел 4 — проработано отдельно).
...
[V] 3.2 Решить вопрос auth (см. раздел 4 — есть рекомендация).
...
--------------------------------------------------------------------------------
[V] Проверка подписи RS256 по public.pem
[V] Проверка exp (и iat), отказ -> 401
[V] Middleware кладёт userId / login / roles в контекст запроса
...
ЧТО ЗАЛОЖИТЬ СРАЗУ (раз это ФУНДАМЕНТ для будущих сервисов, не разовое):
  [V] Topic exchange с осмысленным неймингом routing keys: user.upserted,
      user.deleted (на будущее document.created и т.п.). Exchange НЕ привязывать
      к канбану — он общий.
  [V] Каждый сервис — СВОЯ очередь на нужные routing keys. Канбан слушает user.*.
  [V] Dead-letter queue с самого начала (куда падают необработанные сообщения).
  [V] Идемпотентность consumer: INSERT ... ON CONFLICT DO UPDATE (повторная
...
СТЕК RabbitMQ:
  - Symfony producer: symfony/amqp-messenger + php-ext amqp (УСТАНОВЛЕНЫ).
    Doctrine entity listener на User (postPersist/
...
[V] Схема в БД Go (миграции Go — выбрать инструмент).
...
- 2026-06-29: file_storage/ сделана АВТОНОМНОЙ (под переезд в отдельный репо):
  ...
  поправить data-пути ./../../ -> ./.
- 2026-06-30: РЕАЛИЗОВАН ПЕРЕХОД НА MinIO (S3) И imgproxy В SYMFONY.
  Канбан-вложения переведены с локальной ФС на S3-хранилище (MinIO).
  * Установлен `aws/aws-sdk-php` и зарегистрирован `Aws\S3\S3Client` в DI.
  * `KanbanAttachmentService` переписан на putObject, deleteObject, getObject (StreamedResponse) и exists (headObject).
  * `AttachmentController` и `KanbanAttachmentApiController` переписаны на `StreamedResponse` для скачивания, а превью-ссылки перенаправляют на `imgproxy` URL через 302-редирект.
  * Убран Imagine CacheManager и очищен `liip_imagine.yaml` (удален kanban loader и filter_set).
  * Все изменения верифицированы успешным `lint:container`.
- 2026-06-30: РЕАЛИЗОВАНЫ ЗАДАЧИ 2 И 3 ИЗ БЛИЖАЙШИХ.
  * ЗАДАЧА 2 — JWTCreatedListener (src/EventListener/JWTCreatedListener.php):
    listener на lexik_jwt_authentication.on_jwt_created добавляет claim `id`
    (= User.getId(), int) в JWT payload. claim `username` (=login) и `roles`
    остаются — изменение обратно совместимо. Go берёт id напрямую из токена.
  * ЗАДАЧА 3 — UserSyncEntityListener (src/EntityListener/UserSyncEntityListener.php):
    Doctrine entity listener на User (postPersist/postUpdate/preRemove).
    При изменении РЕЛЕВАНТНЫХ полей (lastname, firstname, patronymic, avatarName,
    deletedAt, login) диспатчит UserSyncMessage через Messenger → AMQP.
    Soft-delete Gedmo перехватывается через postUpdate (deletedAt в WATCHED_FIELDS).
    preRemove — страховка на hard delete.
  * UserSyncMessage (src/Message/UserSyncMessage.php) — readonly DTO:
    event (upserted/deleted), userId, login, lastname, firstname, patronymic,
    avatarName, deletedAt.
  * Транспорт event_bus добавлен в messenger.yaml: AMQP, topic exchange 'events',
    очередь symfony_events с биндингом user.#. AmqpStamp задаёт routing key
    user.upserted / user.deleted. Существующий async (doctrine) не тронут.
  * RABBITMQ_TRANSPORT_DSN добавлен в .env и .env.example
    (amqp://guest:guest@rabbitmq:5672/%2f).
  * Верифицировано: lint:container — OK.
